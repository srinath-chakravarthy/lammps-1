	...
	kappa(0), serialMode(false), compute_pe(NULL), pairEAM(NULL)
{
	...
	if(domain->triclinic)
		error->all(FLERR,"Fix sgcmc does not support non-orthogonal simulation boxes yet.");
...
#if CDEAM_MC_SUPPORT
		if((pairCDEAM = dynamic_cast<PairCDEAM*>(pairEAM))) {
...
void FixSemiGrandCanonicalMC::doMC()
{
	...
	// So we first need to fetch those electron denities for our ghost atoms now.
	...
	for(int i = 0; i < numSamplingWindowMoves; i++) {
		...
		int nDice = (int)(swap_fraction * atom->nlocal / numSamplingWindowMoves);
...
#if SGCMC_DEBUG
				cout << "Old species: " << oldSpecies << " new species: " << newSpecies << " deltaE: " << deltaE << " dm: " << dm << " deltaB: " << deltaB;
#endif
				if(deltaB < 0.0) {
...
double FixSemiGrandCanonicalMC::compute_vector(int index)
{
	...
	if(index  <= atom->ntypes) return (double)speciesCounts[index] / (atom->natoms>0 ? atom->natoms : 1);
